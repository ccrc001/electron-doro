...
export function shareStorePlugin({ store }: PiniaPluginContext): void {
  ...
  let currentStoreUpdateVersion: number = getLocalStoreUpdateVersion(storeCacheVersionKey)
  // 初始化同步store版本
  ...
  store.$subscribe(() => {
    ...
    if (currentStoreUpdateVersion === null || currentStoreUpdateVersion === undefined) {
      ...
      if (storeName != 'loading') {
        // 主动更新
        updateStoreSync(JSON.stringify(store.$state), storeName, store.storeUpdateVersion)
      }
    } else {
