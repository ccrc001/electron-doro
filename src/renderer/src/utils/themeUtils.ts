import themeDarkConfig from '@assets/theme/theme-dark.json'
import themeLightConfig from '@assets/theme/theme-light.json'
...
export const initTheme = () => {
  ...
  window.electron.ipcRenderer.on('theme-style:changed', (_event, mode: string) => {
    // 有mode说明是走主题切换
    if (mode) {
...
export function themeChange(mode: string, electronChange = true) {
  ...
  // 通知主进程告诉其他窗口同步修改主题样式
  if (electronChange) {
    window.electron.ipcRenderer.invoke('theme-style:change', mode)
  }
  ...
  for (let i = 0; i < styleKeys.length; i++) {
    ...
    /// 走缓存或json配置的默认值
    const styleValue: string = cacheUtils.get(keyThemePrefix + styleKey) || styleValues[i]
    /// 设置样式
    setStyleProperty(styleKey, styleValue)
  }
}
